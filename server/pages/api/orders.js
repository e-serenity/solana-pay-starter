...
const orders_namespaceObject = JSON.parse('[{"buyer":"FDo5XApDJtQ8CgCEGGvHMnUTY49S6suQcvfzm4oQEqrF","orderID":"9u7Txy1TUjRKUST13z6eGUbLga7Lm9B59sqfgawUcLBL","itemID":1}]');
...
function get(req, res) {
    ...
    const buyerOrders = orders_namespaceObject.filter((order)=>order.buyer === buyer
    );
...
async function post(req, res) {
    ...
    try {
        ...
        if (!orders_namespaceObject.find((order)=>order.buyer === newOrder.buyer.toString() && order.itemID === newOrder.itemID
        )) {
            orders_namespaceObject.push(newOrder);
            await (0,promises_namespaceObject.writeFile)("./pages/api/orders.json", JSON.stringify(orders_namespaceObject, null, 2));
            res.status(200).json(orders_namespaceObject);
        } else {
